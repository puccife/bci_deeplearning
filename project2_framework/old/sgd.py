...
class SGD:
	...
	def compute_stoch_gradient(self,backward_pass,forward_pass,mini_batch_size):
		...
		for i in range(0, len(backward_pass)):
			...
			weight_loss = torch.mm(backward_pass[i], torch.transpose(forward_pass[i][1], 0,1))  * 1/mini_batch_size
