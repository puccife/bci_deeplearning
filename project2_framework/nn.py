...
def run_network(train_input, train_target, test_input, test_target):
    ...
    for e in range(0, epochs):
        ...
        for b in range(0, shuffle_train.size(0), mini_batch_size):
            ...
            # prediction
            _, predicted_labels = torch.max(forward_pass[len(forward_pass) - 1][1],
                                            0)  # argmax of last layer gets us node with highest proba
            ...
            # backward
            backward_pass = criterion.backward_pass(nn.get_activations(), forward_pass, predicted_labels.float(),
                                                    sample_target, nn.parameters()[0])
