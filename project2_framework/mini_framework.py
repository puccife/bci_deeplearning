...
################ DATA ##################################################
from torch.legacy.nn import LogSoftMax
...
def apply_sigmoid(x):
    ...
    # in order to avoid overflow or underflow caused by
    # large values of input x, we exploit the identity
    # property of the softmax: softmax(x) = softmax(x + constant).
    # Therefore we subtract to the input the max value of the
    # tensor. In this way we have a more stable version of the softmax

    max_val, _ = x.view(-1).max(dim=0)
    exponential = torch.exp(x - max_val)
...
def log_softmax(x):
    """
    :return: the log of the softmax on input x
    """
    return torch.log(apply_sigmoid(x))
...
def forward_layer(input_data, weight, bias):
    s_l = torch.add(torch.mm(weight, input_data),
                    bias)  # compute layerwise z before apply activation : (w(l).x(l-1) + b(l)
...
def forward_pass(input_data, layers, weights, bias):
    layer_data = [(input_data,
                   input_data)]  # add initial input to list storing tuple of layerwise data before and after applying activation per layer
    current_data = input_data
    for layer in range(1, layers):
        current_weight_matrix = torch.transpose(weights[layer - 1], 0, 1)
        current_bias_matrix = (bias[layer - 1]).view(-1, 1)

        node_transformation = forward_layer(current_data, current_weight_matrix,
                                            current_bias_matrix)  # apply calculation of forward pass
...
def last_layer_loss(train_target, predicted_label, last_layer_vec):
    ...
    test = torch.pow(train_target.sub(predicted_label),
                     2)  # compute squared difference between prediction and actual label
...
def compute_stoch_gradient(sample_target, sample_data, weights, bias, layers):
    ...
    ll_delta = last_layer_loss(sample_target, mlp_prediction,
                               layer_data[len(layer_data) - 1][0])  # compute last layer delta
    ...
    for i in range(0, len(backward_data) - 1):
        weight_loss = torch.mm(backward_data[i], torch.transpose(layer_data[i + 1][0], 0,
                                                                 1))  # gradient for sgd w.r.t weight = dot(x(l),delta(l+1).T)
